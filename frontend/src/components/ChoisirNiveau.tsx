...
const ChoisirNiveau: React.FC = () => {
  ...
  const handleLevelSelect = (level: string) => {
    if (level === "debutant") {
      navigate(`/francais/debutant?lang=${lang}`);
    } else if (level === "intermediaire") {
      navigate(`/francais/intermediaire?lang=${lang}`);
    } else if (level === "avance") {
      navigate(`/francais/avance?lang=${lang}`);
    }
  };
